...
from .mtcnn_utils import detect_face, extract_face, detect_face_torch, pil_to_tensor
from utils.image_utils import convert_to_PIL
...
class PNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            ...
            state_dict_path = os.path.join(base_dir, "weights", "mtcnn", "pnet.pt")
...
class RNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            base_dir = os.path.dirname(os.path.dirname(__file__))
            state_dict_path = os.path.join(base_dir, "weights", "mtcnn", "rnet.pt")
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            base_dir = os.path.dirname(os.path.dirname(__file__))
            state_dict_path = os.path.join(base_dir, "weights", "mtcnn", "onet.pt")
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class MTCNN(nn.Module):
    ...
    def __init__(
        ...
        self.original_thresholds = thresholds.copy()
        self.factor = factor
        ...
        # if fr_model is None:
        #     raise ValueError("Please specify whether to use ArcFace alignment or not by setting fr_model='arcface'")
        if fr_model is None:
            self.arcface = False
        elif 'arcface' in fr_model.lower():
            self.arcface = True
        else:
            self.arcface = False
    ...
    def forward(self, img, save_path=None, return_prob=False):
        ...
        ### This function has been added to ensure that a face is detected. ###
        for i, box in enumerate(batch_boxes):
            attempt = 0
            while box is None:
            # Decrease thresholds further each attempt
                self.thresholds = [t * (0.8 ** (attempt + 1)) for t in self.thresholds]
                res = self.detect(img[i], landmarks=True)
                if res[0] is not None and hasattr(res[0], "shape") and res[0].shape[0] > 1:
                    best_idx = np.argmax(res[1])
                    res = ([res[0][best_idx]], [res[1][best_idx]], [res[2][best_idx]])
                batch_boxes[i], batch_probs[i], batch_points[i] = res
                box = batch_boxes[i]
                attempt += 1
            self.thresholds = self.original_thresholds
        ...
        if self.arcface:
            ...
            #     batch_boxes, batch_probs, batch_points, img, method=self.selection_method
            # )
            # Extract faces using ArcFace alignment
            faces = self.extract_face_arcface(img, batch_boxes, batch_points, batch_probs)
        else :
            # Extract faces
    ...
    def extract_face_arcface(self, imgs, boxes, landmarks, probs):
        """Extract and align face from image given bounding box using ArcFace alignment.

        Arguments:
            img {PIL.Image or np.ndarray} -- A PIL image or numpy array.
            box {list or np.ndarray} -- A list or array representing the bounding box
                in the form [x1, y1, x2, y2].
            save_path {str} -- An optional save path for the cropped image.
                (default: {None})
        Returns:
            PIL.Image -- The cropped and aligned face as a PIL image.   
        """

        aligned_tensors = []
        for pil_img, lm, prob in zip(imgs, landmarks, probs):
            if lm is None:
            ...
            aligned_tensor = pil_to_tensor(aligned_face).cuda()
            aligned_tensor = (aligned_tensor - 0.5) / 0.5  # Normalize to [-1, 1]
            aligned_tensors.append(aligned_tensor)

        # aligned_tensors = torch.stack(aligned_tensors).cuda()
    ...
    def extract_landmarks(self, img):
        ...
        # Compute per-element min and max
        min_val = img_flat.min(dim=1, keepdim=True)[0]  # shape: (B, 1)
        max_val = img_flat.max(dim=1, keepdim=True)[0]  # shape: (B, 1)

        # Normalize to [0, 1]
        img_norm = (img_flat - min_val) / (max_val - min_val + 1e-8)

        # Reshape back
        img_norm = img_norm.view(B, C, H, W)

        # img = ((img/2 + 0.5).clamp(0,1)*255)  # to [0, 255]
        img = img_norm.clamp(0,1)*255
        img.to(torch.uint8)
        img = img.permute(0, 2, 3, 1)

        for il in range(len(landmarks)):
            attempt = 0
            while landmarks[il] is None:
            # Decrease thresholds further each attempt
                self.thresholds = [t * (0.8 ** (attempt + 1)) for t in self.thresholds]
                res = self.detect_torch(img[il], landmarks=True)
                if res[2] is not None and hasattr(res[0], "shape") and res[0].shape[0] > 1:
                    best_idx = torch.argmax(res[1])
                    res = (res[0][best_idx], res[1][best_idx], res[2][best_idx])
                    # print(res)
                    # print(res[2].shape)
                    landmarks[il] = res[2].unsqueeze(0)
                attempt += 1
            self.thresholds = self.original_thresholds

        # print(landmarks)
        landmarks = torch.stack(landmarks).float().squeeze(1)
        return landmarks
    ...
    def detect_torch(self, img, landmarks=False):
            """Detect all faces in PIL image and return bounding boxes and optional facial landmarks.
            ...
            batch_boxes, batch_points = detect_face_torch(
                img, self.min_face_size,
                self.pnet, self.rnet, self.onet,
                self.thresholds, self.factor,
                self.device
            ...
            boxes, probs, points = [], [], []
            for box, point in zip(batch_boxes, batch_points):
                box = torch.tensor(box) if not isinstance(box, torch.Tensor) else box
                point = torch.tensor(point) if not isinstance(point, torch.Tensor) else point
                ...
                if box.numel() == 0:
                    ...
                    box_sizes = (box[:, 2] - box[:, 0]) * (box[:, 3] - box[:, 1])
                    box_order = torch.argsort(box_sizes, descending=True)
                    box = box[box_order]
                    point = point[box_order]
                    boxes.append(box[:, :4])
                    probs.append(box[:, 4])
