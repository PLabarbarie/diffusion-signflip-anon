...
from fr_system.resnet import resnet18, resnet34
...
face_labels = [1 if attr in ['skin', 'l_brow', 'r_brow', 'l_eye', 'r_eye', 'eye_g', 'l_ear', 'r_ear', 'nose', 'mouth', 'u_lip', 'l_lip', 'neck'] else 0 for attr in attributes]
# face_labels = [1 if attr in ['skin', 'l_brow', 'r_brow', 'l_eye', 'r_eye', 'eye_g', 'l_ear', 'r_ear',
#  'nose', 'mouth', 'u_lip', 'l_lip', 'neck', 'hair'] else 0 for attr in attributes]

face_labels = torch.tensor(face_labels)
...
class BiSeNet(nn.Module):
    def __init__(self, num_classes, backbone_name="resnet18"):
        ...
        self.face_labels = face_labels.cuda() if torch.cuda.is_available() else face_labels
    ...
    @torch.no_grad()
    def get_face_mask(self, x, size=(256, 256)):
        """
        Get segmentation mask from input image.
        x: (B, 3, H, W), pixel values in [-1, 1]
        """
        if x.dtype != torch.float32 and x.max() >= 128:
            x = x / 255.0 # from [0, 255] to [0, 1]
        elif x.min() < 0.0:
            x = x/2 + 0.5  # from [-1, 1] to [0, 1]
        x = self.transform(x)

        logits, _, _ = self.forward(x)
        masks = torch.argmax(logits, dim=1)
        masks = self.face_labels[masks]
        masks = tfms.Resize(size)(torch.tensor(masks.clone()).unsqueeze(1).float()).repeat(1,3,1,1)
        return masks  # (B, H, W), 0 for non-face, 1 for face
    
    def get_face_logits(self, x):
        """
        Get segmentation logits from input image.
        x: (B, 3, H, W), pixel values in [-1, 1]
        """
        if x.dtype != torch.float32 and x.max() >= 128:
            x = x / 255.0 # from [0, 255] to [0, 1]
        ...
        logits, _, _ = self.forward(x) # (B, num_classes, H, W)
        logits = logits[:, self.face_labels==1, :, :]  # keep only face classes
        logits = torch.sum(logits, dim=1, keepdim=True)  # sum over face classes
