...
class FFHQ(Dataset):
    ...
    def __init__(self, root, transform=None, controlnet=False, controlnet_conditioning_dir=None):
        self.root = root
        ...
        if self.controlnet:
            self.landmark_dir = controlnet_conditioning_dir
            self.latents = torch.load(os.path.join(self.root, "ffhq_latents.pt"))
...
class CelebA_HQ(Dataset):
    ...
    def __init__(self, root, transform=None, clip_image_processor=None):
        ...
        all_files = None
        with open(os.path.join(os.path.dirname(__file__),"celeba_hq_order.txt"), mode = "r") as f:
            all_files = f.read()[1:-1].replace("'", "").replace(" ", "").split(",")
        # I have changed the classical way to access the image as the line bellow
        # The reason is because J-Z has a different sorted list of files so I maintain consistency accross experiments
        # It can be reorder in the "right" order next time Message : 23/01/2026
...
def load_dataset(
    ...
    data_root=".",
    dataset_roots=None,
    controlnet_conditioning_dir = None
):
    dataset_roots = dataset_roots or {}
    if name == "FFHQ":
        root = dataset_roots.get("FFHQ", os.path.join(data_root, "FlickrFace"))
        return FFHQ(root=root, transform=transform, controlnet=controlnet, controlnet_conditioning_dir=controlnet_conditioning_dir)
    elif name == "LFW":
        root = dataset_roots.get("LFW", os.path.join(data_root, "LFW"))
        ...
        root = dataset_roots.get("CelebA", data_root)
        return CelebA(root, split=split, multiple_images=multiple_images, target_type="identity", transform=transform)
    elif name == "CelebA_HQ":
        root = dataset_roots.get("CelebA_HQ", os.path.join(data_root, "CelebAMask-HQ", "CelebA-HQ-img"))
