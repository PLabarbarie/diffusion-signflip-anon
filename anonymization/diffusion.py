...
class DiffusionModel(torch.nn.Module):
    def __init__(
        ...
        inverse_scheduler="hugging_face",
        torch_device="cuda",
        models_root="models",
        ...
        super().__init__()
        self.name = name
        controlnet_weights_root = os.path.join(weights_root, "ffhq_controlnet")
        ...
        if name == "CompVis/ldm-celebahq-256":
            ...
            self.unet = UNet2DModel.from_pretrained(os.path.join(models_root,"ffhq-diffusers"), subfolder="unet", local_files_only=True)
            self.vae = VQModel.from_pretrained(os.path.join(models_root,"ffhq-diffusers"), subfolder="vqvae", local_files_only=True)
            ...
            self.scheduler = DDIMScheduler.from_config("CompVis/ldm-celebahq-256", subfolder="scheduler",
                                                        local_files_only=True)
            unet = UNet2DModel.from_pretrained(os.path.join(models_root,"ffhq-diffusers"), subfolder="unet", local_files_only=True)
            unet_config = unet.config
            del unet
            self.unet = ControlNet(model_config=unet_config,
                        model_ckpt=os.path.join(models_root, "ffhq-diffusers"),
                        hint_channels=3,
                        down_sample_factor=4,
                        device=torch_device)
            state_dict = controlnet_weights_root
    ...
    def conditional_backward_diffusion(self, noisy_latent, condition_type, gt_condition, conditional_model, guidance_scale=1,
    mask = 1, num_inference_steps=50, eta=0.0, save_intermediate_landmarks=True, save_path=None):
        ...
        for t in tqdm.tqdm(self.scheduler.timesteps):
            ...
            if t>0:
                ...
                if condition_type == "landmarks":
                    condition = conditional_model.extract_landmarks(x0_hat)
                    l2 = ((gt_condition - condition)**2).sum((1,2))
                    l2 = torch.log(l2)
                    # print(l2)
                    # xt = xt.detach() - 2 * grad * mask
                elif condition_type == "segmentation": 
                    condition = conditional_model.get_face_logits(x0_hat)
                    # condition = (condition > 0)*1.0 # this line breaks the computation graph

                    bce_loss = torch.nn.BCEWithLogitsLoss(reduction='none')
                    bce = bce_loss(condition, gt_condition).sum((1,2,3))
                    l2 = torch.log(bce)
                    # print(l2)
            ...
            if save_intermediate_landmarks and  t%100 == 0 :
                ...
                plt.clf()
                plt.imshow((x0_hat[0]/2 + 0.5).clamp(0,1).detach().cpu().permute(1,2,0).numpy())
                # Plot landmarks if available
                # print("condition", condition.shape)
                if condition is not None and len(condition) > 0 and condition_type == "landmarks":
                    ...
                    if face_landmarks is not None:
                        ...
                        for landmark in gt_condition[0].detach().cpu().numpy():
                            plt.plot(landmark[0], landmark[1], 'bo', markersize=6)
                elif condition_type == "segmentation":
                    seg_map = ((conditional_model.get_face_mask(x0_hat, size=(256,256))[0,0]>0)*1.0).detach().cpu().numpy()
                    # print('seg_map size', seg_map.shape)
