...
def generate_keys(shape: torch.Size, p: float = 0.5, key_seed: int = 1, device: torch.device = None) -> torch.Tensor:
    """
    ...
    Args:
        ...
        key_seed: int - Seed for random number generator for future keys (default: 1)
    ...
    seeds = torch.randint(0, 2**32, (batch_size,), generator=torch.Generator(device=device).manual_seed(int(key_seed)), device=device)
...
class FlipAnonymizer:
    ...
    guidance: bool = True, guidance_type: str = "landmarks", guidance_scale : float = 1, controlnet : str = None, ddim_inversion : bool = True,
    ...
    def anonymization_pipeline(self, images: torch.Tensor, keys: torch.Tensor, p: float = 0.5, seeds: torch.Tensor = None, return_gaussians : bool = True) -> tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        with torch.no_grad():
            mask = self.face_parser.get_face_mask(images, size=(64,64))
            mask = (mask > 0)*1.0

            if self.controlnet:
                if "landmarks_5" in self.DM.name or "landmarks_26" in self.DM.name:
                    hint = self.face_detector.get_hints(images)
                elif "segmentation_masks" in self.DM.name:
                    hint = self.face_parser.get_face_mask(images, size=(256,256))
                    hint = (hint > 0)*1.0
                elif "multi_controlnet" in self.DM.name:
                    hint_segmentation = self.face_parser.get_face_mask(images, size=(256,256))
                    hint_segmentation = (hint_segmentation > 0)*1.0
                    hint_landmarks = self.face_detector.get_hints(images)
                    hint = (hint_segmentation, hint_landmarks)
                else :
                    raise NotImplementedError("ControlNet type not implemented for anonymization.")
        ...
        if self.guidance:
            if self.guidance_type == "landmarks":
                condition = self.face_detector.extract_landmarks(images)
                condition_network = self.face_detector
            elif self.guidance_type == "segmentation" : 
                condition = self.face_parser.get_face_mask(images, size=(512,512))[:,0:1]
                condition = (condition > 0)*1.0
                condition_network = self.face_parser
            z0_ano, _ = self.DM.conditional_backward_diffusion(z_list, self.guidance_type, condition, condition_network, self.guidance_scale, mask,
    ...
    def de_anonymization_pipeline(self, ano_images: torch.Tensor, keys: torch.Tensor, mask ) -> torch.Tensor:
        ...
        # Encode images to latent space
        if mask is None:
            mask = self.face_parser.get_face_mask(ano_images, size=(64,64))
            mask = (mask > 0)*1.0

        if self.controlnet:
            if "landmarks_5" in self.DM.name or "landmarks_26" in self.DM.name:
                hint = self.face_detector.get_hints(ano_images)
            elif "segmentation_masks" in self.DM.name:
                hint = self.face_parser.get_face_mask(ano_images, size=(256,256))
                hint = (hint > 0)*1.0
            elif "multi_controlnet" in self.DM.name:
                hint_segmentation = self.face_parser.get_face_mask(ano_images, size=(256,256))
                hint_segmentation = (hint_segmentation > 0)*1.0
                hint_landmarks = self.face_detector.get_hints(ano_images)
                hint = (hint_segmentation, hint_landmarks)
            else :
                raise NotImplementedError("ControlNet type not implemented for anonymization.")
